...
This is the ex-script for the task that runs PM (cloud, metar, lightning, pm) preprocess
...
#
if [[ "$run_pm" == true ]]; then
  $APRUN ./${exect}  > stdout_pm 2>&1 || print_err_msg_exit "\
  Call to executable to run PM process returned with nonzero exit code."
